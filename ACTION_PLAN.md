...
- **Additional implementation deviations to reconcile:** Each provider currently has two placeholder throw sites (`_sendInternal` plus a builder), rather than the planned single guard using `isMultiPartPromptPayload`. A compilation-coupling change also added a conversation summary branch in `src/v1/assessor/assessor.service.ts`; this was not in the original scope. Review and minimise both deviations before accepting Section 1. Re-measure LOC, finish public-type documentation and remove temporary test commentary during resumed delivery.
- **Stop/resume instruction:** The user requested an action-plan update, a commit and push of the current work in progress, then no further implementation. Preserve the known RED failures rather than weakening tests or claiming completion. Leave the user's pre-existing agent-configuration edits uncommitted. On resumption, start with the outstanding Section 1 RED corrections, not routing or provider mapping.
...
- **Implementation notes:** filled during delivery.
- **Deviations from plan:** note any departures.
- **Follow-up implications for later sections:** provider sections receive resolved payloads with authoritative `model`/`reasoningEffort`.
