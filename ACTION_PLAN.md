...
- **Implementation notes:** `test/mistral-live.e2e-spec.ts` is a faithful mirror of `assessor-live.e2e-spec.ts`, differing only in the `describe` label, the log path (`logs/mistral-live.e2e-spec.log`), and the `startApp` override pinning both `DEFAULT_TEXT_TABLE_MODEL` and `DEFAULT_IMAGE_MODEL` to `mistral-small-latest`. The `vitest.config.ts` `e2e-live` `include` is now the explicit two-element array `['test/assessor-live.e2e-spec.ts', 'test/mistral-live.e2e-spec.ts']` (a glob was deliberately avoided per SPEC backend change #11). `eslint.config.js` gained `test/mistral-live.e2e-spec.ts` in the `unicorn/prefer-uint8array-base64: 'off'` override list because, like the mirrored live spec, it uses `Buffer.prototype.toString('base64')` for runtime safety (the native `Uint8Array.toBase64()` is unavailable on the deployed Node version — see the comment block at `eslint.config.js:253`). The Gemini live pin in `assessor-live.e2e-spec.ts` (added in Section 8) is intact and unchanged. `npm run build`, `npm run lint`, and `npm run lint:british` all pass. The live suite itself could NOT be runtime-executed in this environment (no `MISTRAL_API_KEY`/`GEMINI_API_KEY` live secrets and no network egress); this is the expected CI gate (`npm run test:e2e:live` requires both keys in `.test.env`). The mock-shape fidelity (covered by Section 8's mocked run) is verified; live execution remains a manual/CI follow-up. No captured response samples were available to refine `llm-mock.mjs`, so the mock placeholder text stands as-is.
- **Deviations from plan:** The plan scheduled the `assessor-live.e2e-spec.ts` Gemini pin for Section 9, but it was actually applied in Section 8 (see Section 8 notes) because the silent-flip regression risk applies as soon as the schema defaults land — no functional deviation, only an earlier commit. Everything else matches the plan.
- **Follow-up implications:** Before merge, run `npm run test:e2e:live` in an environment with both `GEMINI_API_KEY` and `MISTRAL_API_KEY` present in `.test.env` to confirm the live suites pass; if the real Mistral output differs materially from the mocked placeholder text, refine `mistralMockResponse` in `llm-mock.mjs`.
